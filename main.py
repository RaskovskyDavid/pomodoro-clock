...
def start_timer():
    ...
    if reps % 8 == 0:
        ...
        label_time.config(text="Break", fg=PINK)
    else:
        count_down(work_sec)
...
def count_down(count):
    ...
    count_min = count // 60
    count_sec = count % 60
    # canvas.itemconfig(timer_text, text=f"{count_min}:{str(count_sec).ljust(2, '0')}")
    canvas.itemconfig(timer_text, text=f"{count_min}:{str(count_sec).ljust(2, '0')}")
    if count > 0:
...
# Labels
label_time = Label(text="TIMER", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 35, "bold"))
